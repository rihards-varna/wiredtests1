...
from settings import cytec_stand_ip_address,dut_port,all_stands,loop_count,interactive_mode
...
init(autoreset=True)
print(Back.YELLOW + '[4]REBOOT TEST                                                                            ')
...
for i, current_stand in enumerate(all_stands):
    ...
    if "packet-loss=100%" in check_if_alive:
        ...
        ssh.close()
    #! DUT_REBOOT TEST
    for t in range(0,loop_count):
        print("------------------------------------------------------------------------------------------")
        print(Style.BRIGHT + f'LOOP {t+1}/{loop_count}')
        print("------------------------------------------------------------------------------------------")
        ...
        # ! CHECK LINK-STATUS AND PING WHEN READY
        #? LOGIN INTO STAND
        print('[SECURE SHELL]')
        print("Logging into DUT..")
...
results_4 = return_results()
#?SAVE IN JSON
with open("results_4.json", "w") as f:
    json.dump(results_4, f)
