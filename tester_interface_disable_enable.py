...
init(autoreset=True)
print(Back.YELLOW + '[2]TESTER INTERFACE DISABLE/ENABLE TEST                                                   ')
...
for i, current_stand in enumerate(all_stands):
    ...
    #CHECKING IF INITIAL PING WORKS
    print("Logging into STAND..")
    ssh = paramiko.SSHClient() 
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy()) 
    ssh.connect(current_stand, username="admin", password="")
    ...
    # STAND(TESTER)INTERFACE DISABLE/ENABLE TEST
    for t in range(0,3):
        print("------------------------------------------------------------------------------------------")
        ...
        print(f"Executing command '/interface/disable {stand_port}; delay 5s; interface/enable {stand_port};'")
        ssh.exec_command(f'/interface/disable {stand_port}; delay 5s; interface/enable {stand_port};\r\n')
        time.sleep(6.0)
        print(f"DONE")
        ssh.close()
        
        #? LOGIN BACK INTO STAND
        print('[SECURE SHELL]')
        print("Logging into DUT..")
