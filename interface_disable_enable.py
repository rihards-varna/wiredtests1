...
# #USER INPUT
# dut_port = input("DUT interface: ")
# stand_port = input("Stand interface: ")

# while True:
#     try:
#         dut_count = int(input("DUT count: "))
#         break
#     except ValueError:
...
init(autoreset=True)
print(Back.YELLOW + '[1]INTERFACE DISABLE/ENABLE TEST                                                          ')
...
def pause_test():
    if interactive_mode == True:
        while True:
            resume_test = input('Test paused, press "y" to continue: ').strip().lower()
            if resume_test == 'y':
                break
            else:
