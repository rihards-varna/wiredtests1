...
# ! TESTAM JAPRABAUDA VAI INTERRVALA STARP MANUALO DISABLE/ENABLE UN PING TEST FINISH NENOTIEK NEPAREDZETS LINK-DOWNs. 

from colorama import Fore,Back,Style,init
init(autoreset=True)
print(Back.YELLOW + '[3]INTERFACE DISABLE/ENABLE WHILE SENDING TRAFFIC                                         ')
...
for i, current_stand in enumerate(all_stands):
    ...
    time.sleep(0.5)
    ssh.exec_command(f'/tool traffic-gene stream add name=send_traffic id=0 tx-template=send_traffic packet-size=64-1500 mbps=500')
    ...
    if "packet-loss=100%" in check_if_alive:
        ...
        a.send(f'/ip address add address=192.168.1.2/26 interface={dut_port}\r\n')
    ...
    for t in range(0, 3):
        print("------------------------------------------------------------------------------------------")
        print(Style.BRIGHT + f'LOOP {t+1}/3')
        print("------------------------------------------------------------------------------------------")
        #? LOGIN INTO DUT
        print('[SECURE SHELL]')
        print("Logging into DUT..")
    ...
    ssh.connect(current_stand, username="admin", password="")
    print("DONE")
    print(f"Stopping traffic generation..")
    ssh.exec_command('/tool traffic-generator stop')
    print("DONE")
...
results_3 = return_results()
#?SAVE IN JSON
with open("results_3.json", "w") as f:
    json.dump(results_3, f)
