...
init(autoreset=True)
print(Back.YELLOW + '[5]MAX MTU & L2MTU TEST                                                                   ')
        ...
        try:
            ...
            if stand_port + ' ' in stand_link_status:
                ...
                for attempt in range(0, 3):
                    try:
                        stdin, stdout, stderr = ssh.exec_command("ping 192.168.1.2 count=10")
                        ping_output = stdout.read().decode()
                        
                        if "received=10" in ping_output or "packet-loss=0%" in ping_output:
                            print(Fore.GREEN + f'Ping attempt {attempt + 1} successful 10/10')
                            ping_success = True
                            break
                        else:
...
for i, current_stand in enumerate(all_stands):
    ...
    # ! MAX-MTU TEST
    print("------------------------------------------------------------------------------------------")
    print(Style.BRIGHT + f'SETTING MAX MTU & L2MTU')
    print("------------------------------------------------------------------------------------------")
    ...
    stdin, stdout, stderr = ssh.exec_command(f'{{:local interface "{dut_port}";:local l2mtu [/int get $interface max-l2mtu];:put $l2mtu}}')
    dut_max_mtu = stdout.read().decode()
    int_dut_max_mtu = int(dut_max_mtu)
    print(f"Executing command '/interface/ethernet/set {dut_port} mtu={int_dut_max_mtu} l2mtu={int_dut_max_mtu}'")
    ssh.exec_command(f'/interface/ethernet/set {dut_port} mtu={int_dut_max_mtu} l2mtu={int_dut_max_mtu}')
    print("DONE")
    ssh.close()
    ...
    print(f"Executing command '/interface/ethernet/set {stand_port} mtu={int_dut_max_mtu} l2mtu={int_dut_max_mtu}'")
    ssh.exec_command(f'/interface/ethernet/set {stand_port} mtu={int_dut_max_mtu} l2mtu={int_dut_max_mtu}') 
    time.sleep(3)
    print(f"DONE")
    
    wait_link_ok_and_ping()
    ssh.close()
    
    print("------------------------------------------------------------------------------------------")
    print(Style.BRIGHT + 'DISABLE/ENABLE TEST')
    print("------------------------------------------------------------------------------------------")
    ...
    print(f"Executing command '/interface/disable {dut_port}; delay 5s; interface/enable {dut_port};'")
    ssh.exec_command(f'/interface/disable {dut_port}; delay 5s; interface/enable {dut_port};\r\n')
    time.sleep(3.0)
    print("DONE")
    ssh.close()
    ...
    print("------------------------------------------------------------------------------------------")
    print(Style.BRIGHT + 'REBOOT TEST')
    print("------------------------------------------------------------------------------------------")
    ...
    try:
        print(f"Executing command '/system reboot'")
        ...
        print("SSH session no longer active")
...
results_5 = return_results()
#?SAVE IN JSON
with open("results_5.json", "w") as f:
    json.dump(results_5, f)
